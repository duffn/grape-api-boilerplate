...
describe JwtTokenService do
  ...
  it 'decodes a JWT token' do
    decoded_data = described_class.new.decode(valid_token)
  ...
  it 'fails on an invalid JWT token' do
    expect { described_class.new.decode(invalid_token) }.to raise_error(JWT::DecodeError)
  end

  it 'fails on an expired JWT token' do
    expect { described_class.new.decode(expired_token) }.to raise_error(JWT::ExpiredSignature)
  end

  it 'fails on an incorrect iss claim' do
    expect { described_class.new.decode(incorrect_iss_token) }.to raise_error(JWT::InvalidIssuerError)
  end
end
